<?php
$formatedPrice = number_format($product['price'], 0);
echo "
<section class='filled card'>
    ...
    <div class='card-body'>
        ...
        <div class='secondary-title'>{$formatedPrice} ریال</div>
        <p class='card-text'></p>
        <a href='{$controllerroot}addCard_controller.php?id={$product['id']}' class='btn btn-primary {$cardclass} card_btn'>{$cardtext}</a>
    </div>
</section>
"
?>
